    ...
    public class CardController : Controller
    {
        ...
        public async Task<IActionResult> Checkout(OrderDto orderDto)
        {
            if (ModelState.IsValid)
            {
                ...
                // 1) Kredi kartı numarası kotnrolü (algoritma)
                // 2) eğer kart numaras geçerli ise ödemeyi al
                // 3) Ödeme başarılıysa order'ı al
                // 4)
                if (!CardNumberControl(orderDto.CardNumber))
                {
                    TempData["Message"] = Jobs.CreateMessage("Hata", "Kredi kart numaras hatalıdır.", "danger");
                    return View(orderDto);
                }
        ...
        private bool CardNumberControl(string cardNumber)
        {
             ...
             *  - cardNumber'ın uzunluk kontrolü
        ...
        [NonAction]
        private Payment PaymentProcess(OrderDto orderDto) //iyzco
        {
            #region PaymetOptionsCreated

            Options options = new Options();
            options.ApiKey = "sandbox-5A39hm5JcjrWxbh3a5tAUhfAwJLhyhSz";
            options.SecretKey = "sandbox-ZPnJfAGOI3cSsEqYkBTVnmgPvXqygijX";
            ...
            CreatePaymentRequest request = new CreatePaymentRequest
            {
                ...
                Buyer = new Buyer
                {
                    ...
                    Name = "Wissen",
                    Surname = "Akademie",
                    GsmNumber = "+905226255889",
                    Email = "wissen@gmail.com",
                    IdentityNumber = "750088009457",
                    RegistrationAddress = "Barbaros Bulvarı Yldız İş Merkezi KAt:3",
                    Ip = "85.34.121.78",
                    City = "İstanbul",
                    Country = "Türkiye"
                },
                ShippingAddress = new Address
                {
                    ...
                    City = "istanbul",
                    Country = "Türkiye",
                    Description = "Barbaros Bulvarı Yldız İş Merkezi KAt:3"
            ...
            foreach (var item in orderDto.CardDto.CardItems)
            {
                ...
                basketItem.Name = item.CardItemId.ToString();
        ...
        {
            var order = await _orderManager.GetProductDetailsByUrlAsync(producturl);
