    ...
    public class UserController : Controller
    {
        ...
        public async Task<IActionResult> Edit(UserAddDto userUpdateDto)
        {
            if(ModelState.IsValid)
            {
                ...
                var userRoles = await _userManager.GetRolesAsync(user); //userın seçili rollerini alır

                await _userManager.AddToRolesAsync(
                    user, userUpdateDto.SelectedRoles.Except(userRoles).ToList<string>() //userın rollerini çkarır, yeni eklenenleri alr
                    );
                await _userManager.RemoveFromRolesAsync(
                    user,
                    userRoles.Except(userUpdateDto.SelectedRoles).ToList<string>()
                    );
                TempData["Message"] = Jobs.CreateMessage("BAşarılı", $"{user.UserName} kullancısı başarıyla güncellendi", "success");
                return RedirectToAction("Index", "User");
            }
        ...
        //ÖDEV : Roller ve kullanıcılar ile ilgili silme işlemlerini de tamamlayn
    }
}
