...
namespace ShoppingApp.Core
{
    public static class Jobs
    {
        ...
        public static string UploadImage(IFormFile image)
        {
            ...
            var path = Path.Combine(Directory.GetCurrentDirectory(), "wwwroot/images", randomName); //resmin tam yolunu iafde eder.
            //http://localhost:5001/"wwwroot/images"/564sdsdrys5485.png

            using (var stream = new FileStream(path, FileMode.Create)) //verilmiş olan path'e yeni bir dosya yükler
            {
                image.CopyTo(stream); 
            }
            return randomName; //IFormFile doyasını alıcak, image klasörüne kaydedecek.
        }
    }
}
